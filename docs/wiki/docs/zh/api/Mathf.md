...
| 名称 | 类型 | 描述 |
|------|------|------|
| PI | `float` | π（3.14159...）——数学界的摇滚巨星。 *(只读)* |
| TAU | `float` | The value of tau (2 * pi). *(只读)* |
| Infinity | `float` | 正无穷。 *(只读)* |
| NegativeInfinity | `float` | Negative infinity. *(只读)* |
| Epsilon | `float` | 极小正数。 *(只读)* |
| Deg2Rad | `float` | 度转弧度系数。 *(只读)* |
| Rad2Deg | `float` | 弧度转度系数。 *(只读)* |
