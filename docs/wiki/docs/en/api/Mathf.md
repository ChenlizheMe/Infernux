...
|------|------|------|
| PI | `float` | The value of pi (3.14159...). *(read-only)* |
| TAU | `float` | The value of tau (2 * pi). *(read-only)* |
| Infinity | `float` | Positive infinity. *(read-only)* |
| NegativeInfinity | `float` | Negative infinity. *(read-only)* |
| Epsilon | `float` | The smallest float value greater than zero. *(read-only)* |
| Deg2Rad | `float` | Degrees-to-radians conversion constant. *(read-only)* |
| Rad2Deg | `float` | Radians-to-degrees conversion constant. *(read-only)* |
