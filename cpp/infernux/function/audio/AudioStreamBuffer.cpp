...
bool AudioStreamBuffer::ReadFrame(uint64_t frame, float &left, float &right)
{
    for (auto &page : m_pages) {
        int expected = Ready;
        if (!page.state.compare_exchange_strong(expected, Reading, std::memory_order_acquire))
            continue;
        const bool found = frame >= page.start && frame - page.start < page.count;
        if (found) {
            const auto offset = static_cast<size_t>(frame - page.start) * 2;
            left = page.samples[offset];
            right = page.samples[offset + 1];
        }
...
bool AudioStreamBuffer::Fill(uint64_t frame)
{
    ...
    for (uint64_t n = 0; n < count; ++n) {
        ...
        for (auto &p : m_pages)
            if (p.state.load(std::memory_order_acquire) != Empty && p.start == start)
                exists = true;
        if (exists)
            continue;
        Page *target = nullptr;
        for (auto &p : m_pages) {
            ...
            if (state != Empty && (state != Ready || wanted(p.start)))
                continue;
            if (p.state.compare_exchange_strong(state, Writing, std::memory_order_acquire)) {
                target = &p;
                break;
            }
        }
        if (!target)
            return false; // An obsolete page is briefly held by the callback.
        try {
            ...
            while (read < frames) {
                ...
                if (!got)
                    throw std::runtime_error("Audio stream ended before its declared frame count");
                read += got;
            }
            ...
            if (m_mono && from.channels > 1) {
                for (size_t i = 0; i < frames; ++i) {
                    float sum = 0;
                    for (int c = 0; c < from.channels; ++c)
                        sum += m_decode[i * from.channels + c];
            ...
            if (!SDL_ConvertAudioSamples(&from, reinterpret_cast<const Uint8 *>(m_decode.data()),
                                         static_cast<int>(frames * from.channels * sizeof(float)), &to, &converted,
                                         &bytes))
                throw std::runtime_error(SDL_GetError());
