...
AudioStreamDecoder::AudioStreamDecoder(const std::string &path) : m_impl(std::make_unique<Impl>())
{
    ...
    if (ext == ".wav") {
        ...
        if (!drmp3_seek_to_pcm_frame(&d.mp3, 0))
            throw std::runtime_error("Cannot rewind MP3: " + path);
    } else if (ext == ".flac") {
#ifdef _WIN32
...
#endif
        if (!d.flac)
            throw std::runtime_error("Cannot open FLAC: " + path);
...
#endif
        if (!input)
            throw std::runtime_error("Cannot open Ogg: " + path);
        int error = 0;
        d.vorbis = stb_vorbis_open_file(input, true, &error, nullptr);
        if (!d.vorbis)
            throw std::runtime_error("Cannot decode Ogg: " + path);
        d.kind = Impl::Kind::Vorbis;
        const auto info = stb_vorbis_get_info(d.vorbis);
        d.frames = stb_vorbis_stream_length_in_samples(d.vorbis);
...
uint64_t AudioStreamDecoder::FrameCount() const
{
    return m_impl->frames;
}
int AudioStreamDecoder::SampleRate() const
{
    return m_impl->rate;
}
int AudioStreamDecoder::Channels() const
{
...
size_t AudioStreamDecoder::Read(float *out, size_t frames)
{
    ...
    switch (d.kind) {
    case Impl::Kind::Wav:
        return drwav_read_pcm_frames_f32(&d.wav, frames, out);
    case Impl::Kind::Mp3:
        return drmp3_read_pcm_frames_f32(&d.mp3, frames, out);
    case Impl::Kind::Flac:
        return drflac_read_pcm_frames_f32(d.flac, frames, out);
    case Impl::Kind::Vorbis:
        return stb_vorbis_get_samples_float_interleaved(
            d.vorbis, d.channels, out, static_cast<int>(std::min(frames, size_t(INT_MAX / d.channels))) * d.channels);
    default:
        throw std::logic_error("Audio decoder not open");
    }
}
void AudioStreamDecoder::Seek(uint64_t frame)
{
    auto &d = *m_impl;
    if (frame >= d.frames)
        throw std::out_of_range("Audio seek outside source");
    bool ok = false;
    switch (d.kind) {
    case Impl::Kind::Wav:
        ok = drwav_seek_to_pcm_frame(&d.wav, frame);
        break;
    case Impl::Kind::Mp3:
        ok = drmp3_seek_to_pcm_frame(&d.mp3, frame);
        break;
    case Impl::Kind::Flac:
        ok = drflac_seek_to_pcm_frame(d.flac, frame);
        break;
    case Impl::Kind::Vorbis:
        ok = stb_vorbis_seek(d.vorbis, static_cast<unsigned>(frame));
        break;
    default:
        break;
    }
    if (!ok)
        throw std::runtime_error("Audio decoder seek failed");
}
} // namespace infernux
