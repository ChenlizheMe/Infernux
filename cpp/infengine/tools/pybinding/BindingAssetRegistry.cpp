...
{
    // ── InfMesh — read-only runtime mesh asset (Phase 0) ────────────────
    py::class_<InfMesh, std::shared_ptr<InfMesh>>(m, "InfMesh")
        .def_property_readonly("name", &InfMesh::GetName, "Mesh asset name")
        .def_property_readonly("guid", &InfMesh::GetGuid, "Mesh asset GUID")
        .def_property_readonly("file_path", &InfMesh::GetFilePath, "Source file path")
        .def_property_readonly("vertex_count", &InfMesh::GetVertexCount, "Total vertex count")
        .def_property_readonly("index_count", &InfMesh::GetIndexCount, "Total index count")
        .def_property_readonly("submesh_count", &InfMesh::GetSubMeshCount, "Number of submeshes")
        .def_property_readonly("material_slot_count", &InfMesh::GetMaterialSlotCount, "Number of material slots")
        .def_property_readonly("material_slot_names", &InfMesh::GetMaterialSlotNames,
                               "Material slot names from model file")
        .def(
            "get_bounds",
            [](const InfMesh &self) -> py::tuple {
        ...
        .def(
            "get_submesh_info",
            [](const InfMesh &self, uint32_t index) -> py::dict {
        ...
        .def("__repr__", [](const InfMesh &self) {
            return "<InfMesh '" + self.GetName() + "' " + std::to_string(self.GetVertexCount()) + " verts, " +
    ...
    py::class_<AssetRegistry, std::unique_ptr<AssetRegistry, py::nodelete>>(m, "AssetRegistry")
        ...
        .def(
            ...
            [](AssetRegistry &self, const std::string &path) {
                return self.LoadAssetByPath<InfMaterial>(path, ResourceType::Material);
            },
        ...
        .def(
            ...
            [](AssetRegistry &self, const std::string &guid) {
                return self.LoadAsset<InfMaterial>(guid, ResourceType::Material);
            },
            py::arg("guid"), "Load a material by its GUID")
        .def(
            "get_material",
            [](AssetRegistry &self, const std::string &guid) { return self.GetAsset<InfMaterial>(guid); },
        ...
        .def(
            "load_mesh",
            [](AssetRegistry &self, const std::string &path) {
                return self.LoadAssetByPath<InfMesh>(path, ResourceType::Mesh);
            },
            py::arg("path"), "Load a mesh by file path (.fbx, .obj, .gltf, …)")
        .def(
            "load_mesh_by_guid",
            [](AssetRegistry &self, const std::string &guid) {
                return self.LoadAsset<InfMesh>(guid, ResourceType::Mesh);
            },
            py::arg("guid"), "Load a mesh by its GUID")
        .def(
            "get_mesh", [](AssetRegistry &self, const std::string &guid) { return self.GetAsset<InfMesh>(guid); },
