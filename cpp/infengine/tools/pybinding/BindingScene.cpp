...
static GameObject *CreateModelObject(Scene *scene, const std::string &guid, const std::string &name = "")
{
    ...
    auto mesh = registry.LoadAsset<InfMesh>(guid, ResourceType::Mesh);
    if (!mesh)
        return nullptr;
...
void RegisterSceneBindings(py::module_ &m)
{
    ...
    py::class_<MeshRenderer, Component>(m, "MeshRenderer")
        ...
        .def(
            "has_render_material", [](const MeshRenderer &mr) -> bool { return mr.GetMaterial(0) != nullptr; },
            "Check if a custom material is assigned (slot 0)")
        .def_property(
            "render_material", [](const MeshRenderer &mr) { return mr.GetMaterial(0); },
            [](MeshRenderer &mr, std::shared_ptr<InfMaterial> mat) { mr.SetMaterial(0, std::move(mat)); },
            "The material used for rendering (slot 0)")
        .def_property(
            "material_guid", [](const MeshRenderer &mr) -> std::string { return mr.GetMaterialGuid(); },
            [](MeshRenderer &mr, const std::string &guid) { mr.SetMaterial(0, guid); }, "The material GUID for slot 0")
        .def(
        ...
        .def(
            "set_material",
            [](MeshRenderer &mr, uint32_t slot, const std::string &guid) { mr.SetMaterial(slot, guid); },
            py::arg("slot"), py::arg("guid"), "Set material at slot index by GUID")
        .def(
        ...
        .def(
            "get_mesh_asset",
            [](const MeshRenderer &mr) -> std::shared_ptr<InfMesh> {
                if (mr.HasMeshAsset())
                    return mr.GetMeshAssetRef().Get();
                return nullptr;
            },
            "Get the InfMesh asset object (None if no asset mesh)")
