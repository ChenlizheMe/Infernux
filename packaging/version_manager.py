"""InfEngine version manager — downloads & caches engine wheels from GitHub Releases.
...
Layout on disk::
    ...
    ~/.infengine/
        versions/
            0.2.0/
                infengine-0.2.0-cp312-cp312-win_amd64.whl
            0.3.0/
                infengine-0.3.0-cp312-cp312-win_amd64.whl
"""
...
@dataclass
class EngineVersion:
    """Represents a single InfEngine release."""
...
class VersionManager:
    """Discovers, downloads, and manages InfEngine engine versions."""
    ...
    def get_wheel_path(self, version: str) -> Optional[str]:
        ...
        if not ver_dir.is_dir():
            return None
        wheels = glob.glob(str(ver_dir / "infengine-*.whl"))
    ...
    def install_local_wheel(self, wheel_path: str) -> str:
        ...
        match = re.match(r"infengine-([^-]+)-", filename, re.IGNORECASE)
        if not match:
            raise ValueError(
                f"Cannot determine version from wheel filename: {filename}\n"
                "Expected a file like infengine-0.3.0-cp312-cp312-win_amd64.whl"
            )
    ...
    def _local_versions(self) -> List[str]:
        ...
        for entry in _VERSIONS_DIR.iterdir():
            if entry.is_dir() and not entry.name.startswith("_"):
                wheels = glob.glob(str(entry / "infengine-*.whl"))
                if wheels:
...
def _find_wheel_asset(release: dict) -> tuple[str, int]:
    ...
    for asset in release.get("assets", []):
        name = asset.get("name", "")
        if name.endswith(".whl") and "infengine" in name.lower():
